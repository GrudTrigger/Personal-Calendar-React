...
const DayShowComponent = ({
  ...
  useEffect(() => {
    if (ref.current) {
      setHeightDiv(ref.current.clientHeight / hoursOfDay);
    }
  }, []);
  ...
  const isDayContaintCurrentTimestamp = (x, b) => {
    return (
      x >= b.startOf("day").format("X") &&
      x <= b.clone().endOf("day").format("X")
    );
  };
  ...
  return (
    <div className="flex justify-between pt-4">
      <div
        className=" w-[665px] h-full flex flex-col pl-1 pr-1 relative"
        ref={ref}
      >
        {isDayContaintCurrentTimestamp(moment().format("X"), today) ? (
          <div
            className="bg-red-600 h-[1px] absolute left-0 right-0"
        ...
        {cells.map((eventsList, i) => (
          <div key={i} className=" flex-grow relative border-t ml-8">
            ...
            <div className=" h-8 min-h-full pt-1"></div>
          </div>
        ))}
        {eventsForDay.map((event, index) => {
          return (
            <span
              className="ml-2 px-1 bg-sky-700 text-white rounded cursor-pointer absolute top-0"
              style={{
                left: 32 + (eventWidth + 1) * index,
                height:
                  heightDiv *
                  (event.duration - moment.unix(+event.date).hour()),
